...
class TodoViewController: UIViewController {
    ...
    static let stortboardID: String = "TodoViewController"
    ...
    @objc private func touchUpCancelButton(_ sender: UIBarButtonItem) {
        if self.todo == nil {
            //이전화면에서 전달받은 todo가 없다면 새로 작성을 위한 상태이므로 모달을 내려주고
    ...
    @objc private func touchUpDoneButton(_ sender: UIBarButtonItem) {
        //todo제목은 필수사항이므로 입력했는지 확인
        guard let title: String = self.titleField.text,
              title.isEmpty == false else {
            self.showSimpleAlert(message: "제목은 꼭 작성해야 합니다",
        ...
        todo = Todo(title: title,
                    ...
                    id: self.todo?.id ?? String(Date().timeIntervalSince1970))
    ...
    override func viewDidLoad() {
        ...
        //이전화면에서 전달받은 todo가 없다면 새로운 작성화면 설정
        if self.todo == nil {
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        //수정모드라면 텍스트 필드에 바로 입력할 수 있도록 키보드 보여줌
        if self.mode == Mode.edit {
            self.self.titleField.becomeFirstResponder()
        }
    }
}
